...
def take_quiz(request, level_number):
    ...
    # For regular quizzes (non-Basic Facts), show questions one by one like measurements
    if not is_basic_facts:
        # Get Quiz topic
        quiz_topic, _ = Topic.objects.get_or_create(name="Quiz")
        
        # Get all questions for this level from database (all topics)
        all_questions_query = level.questions.all()
        ...
        timer_session_key = "quiz_timer_start"
        questions_session_key = "quiz_question_ids"
        timer_start = request.session.get(timer_session_key)
        
        # Clear session if questions don't match (safety check)
        ...
        if completed:
            ...
            if total_points == 0 and student_answers.exists():
                ...
                answered_questions = Question.objects.filter(id__in=answered_question_ids, level=level)
